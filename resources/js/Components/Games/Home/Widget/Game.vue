...
const date = computed(() => formatDate(props.game.date, {
    day: '2-digit',
    month: '2-digit',
    year: 'numeric'
}).replace(/\//g, '.'));
...
<template>
    <div class="group/item bg-surface-100 dark:bg-surface-800 lg:mx-0.5 2xl:mx-1.5 h-full flex flex-col ">
        ...
        <div class="relative flex-grow">
            ...
            <div class="flex flex-row py-3 px-5 items-start pt-10" v-else>
            ...
            <div class="overlay group-hover/item:opacity-100" v-if="game.status !== 'pending'">
                <a :href="route('national-league.game', {'slug': game.slug})">
                    <Button severity="primary" label="Ver estadísticas"/>
                </a>
            </div>
        </div>
        <div
...
.overlay a, .overlay a:active {
    @apply relative top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 inline-block;
}
</style>
